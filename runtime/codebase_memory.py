...
GENERIC_FUNC_PATTERNS = {
    ...
    "typescript": [],
    "java": [re.compile(r"^(?!\s*(?:return|throw|new|if|for|while|switch|catch|assert|yield)\b)\s*(?:(?:public|protected|private|static|final|synchronized|abstract|native|strictfp|default)\s+)*[\w<>\[\], ?.@]+\s+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:throws\s+[^;{]+)?\s*(?:\{|$)")],
    ...
    "c": [re.compile(r"^(?!\s*(?:if|for|while|switch|return|sizeof|throw|new|delete|case|else|do)\b)\s*(?:[\w*&:<>,\[\]\s]+)\s+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:\{|$)")],
    "cpp": [],
    "csharp": [re.compile(r"^(?!\s*(?:return|throw|if|for|foreach|while|switch|catch|using|lock|yield|await)\b)\s*(?:(?:public|protected|private|internal|static|virtual|override|async|sealed|abstract|extern|unsafe|new|partial)\s+)*[\w<>\[\],?.]+\s+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:\{|$)")],
